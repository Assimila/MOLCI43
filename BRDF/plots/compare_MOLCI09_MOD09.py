...
def compare(mod09, predicted_refl_molci43,
    ...
    fig.text(0.5, 0.04, 'SIAC - OLCI %d DoY %s' % (year, strDoY), ha='center')
...
for DoY in [160]:
    ...
    DataDir = '/data/Sentinel3/OLCI/h17v05/SIAC09GA/2017/plots'
    #DataDir = '/data/Sentinel3/OLCI/h17v05/SIAC09GA/2017/plots/MOLCI43_MODIS_and_OLCI'

    # Get OLCI09
    OLCI_DataDir = '/data/Sentinel3/OLCI/h17v05/SIAC09GA/2017'
    OLCI_date = '20170607_1030'
    olci09 = np.zeros((5,2400,2400), np.float32)
    for i, b in enumerate([1, 2, 3, 4, 7]):
        fname = os.path.join(OLCI_DataDir, 'MODIS_h17v05_%s_band%d_sur.tif' % (OLCI_date, b))
        d_olci09 = gdal.Open(fname)
        olci09[i] = d_olci09.ReadAsArray()
        olci09[i] = np.where(olci09[i] == -9999, 0, olci09[i])
        olci09[i] = olci09[i] * 0.0001

    kernels_fname = "MODIS_h17v05_%s_kernels.tif" % (OLCI_date)
    #kernels_fname = "%s.%d%s.%s.kernels.tif" % (product, year, strDoY, tile)
    kernels_fname = os.path.join(OLCI_DataDir, kernels_fname)
    ...
    output_fname = os.path.join(DataDir,
                       'MOLCI43.MODIS_only.%s.predRefl.%d%s.tif' % (product, year, strDoY))
    save_file(predicted_refl_molci43, output_fname, LandMask)

    output_fname = os.path.join(DataDir,
                       'MCD43.MODIS_only.%s.predRefl.%d%s.tif' % (product, year, strDoY))
    save_file(predicted_refl_mcd43, output_fname, LandMask)

    # Plot
    compare(olci09, predicted_refl_molci43,
            predicted_refl_mcd43,
